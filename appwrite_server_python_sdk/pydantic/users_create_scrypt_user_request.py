...
class UsersCreateScryptUserRequest(BaseModel):
    ...
    password_cpu: int = Field(alias='passwordCpu')

    # Optional memory cost used to hash password.
    password_memory: int = Field(alias='passwordMemory')

    # Optional parallelization cost used to hash password.
    password_parallel: int = Field(alias='passwordParallel')
